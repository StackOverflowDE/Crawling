...
def git_crawling(language, your_id, your_password):
    ...
    driver.get("https://github.com/topics/{}?o=desc&s=stars".format(language))
    ...
    for repo_list in repo_git_list[:-1]:
        ...
        try:
            # 이미지 요소 찾기
            img_element = driver.find_element(By.XPATH, '//*[@id="repo-title-component"]/img')
            # 이미지 소스 URL 가져오기
    ...
    # assets/data/git 폴더가 없으면 생성
    folder_git = os.path.join("assets","data","git")
    if not os.path.exists(folder_git):
        os.makedirs(folder_git)

    # CSV 파일로 저장
    csv_file_path = os.path.join("assets", "data", "git", "git_info_{}.csv".format(language))

    # CSV 파일로 저장
    with open(csv_file_path, 'w', newline='', encoding='utf-8') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=["title","fork","stars","watch","recent_time","writer","url","img"])
        writer.writeheader()
